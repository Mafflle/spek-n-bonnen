...
export const actions: Actions = {
	'manage-manufacturer': async ({ fetch, request }) => {
		...
		try {
			...
			if (id) {
				const editManufacturer = await fetch(
					`${PUBLIC_API_ENDPOINT}api/inventory/manufacturers/${id}/`,
					{
						method: 'PUT',
				...
				const createManufacturer = await fetch(
					`${PUBLIC_API_ENDPOINT}api/inventory/manufacturers/`,
					{
						method: 'POST',
						body: JSON.stringify(validatedData)
					}
