...
const resetSchema = z
	...
	export const load: PageServerLoad = async ({url, fetch}) => {
		...
		if (token) {
				const res = await fetch(`${PUBLIC_API_ENDPOINT}api/auth/password-reset/validate_token/`, {
				method: 'POST',
        body: JSON.stringify({token})
			...
			if (!res.ok) {
				throw redirect(302, 'forgot-password?message=Token has expired, get a new one here&type=error')
			}
		} 
	};
...
export const actions: Actions = {
  resetpassword:async ({request, fetch, url, route}) => {
		...
		try {
				if (!token) {
			throw redirect(302, 'forgot-password?message=No token provided, get a new one here&type=error')
		}
      resetSchema.parse(dataToValidate);
      const validatedData = dataToValidate
			delete validatedData.confirmPassword
			
			const res = await fetch(`${PUBLIC_API_ENDPOINT}api/auth/password-reset/confirm/`, {
				method: 'POST',
