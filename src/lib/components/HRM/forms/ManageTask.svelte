<script lang="ts">
	...
	let start_time = task?.start_time || null;
	let end_time = task?.end_time || null;
	let loading: boolean = false;
	const dispatch = createEventDispatcher();

	let taskName = task?.title || '';
	let taskDescription = task?.description || '';
	let taskPriority = { value: task?.priority || 'NORMAL', label: task?.priority || 'Normal' };

	ManagersAssigned.set(task?.assignees || []);

	$: times = { startTime: task?.start_time, endTime: task?.end_time };
